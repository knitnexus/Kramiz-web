                        ...
                        <div className="bg-gradient-to-br from-[#008069] to-[#00a884] p-4 rounded-2xl shadow-lg shadow-green-100 flex items-center justify-between gap-4 border border-white/20">
                            <div className="flex-1">
                                <h4 className="text-white font-black text-sm uppercase tracking-wider flex items-center gap-2">
                                    <span className="text-lg">✨</span> AI Smart Invoice Scan
                                </h4>
                                <p className="text-green-50 text-[11px] font-medium leading-tight mt-1">Upload a photo of a sales invoice and I'll extract the items for you.</p>
                            </div>
                                ...
                                <button 
                                    onClick={async () => {
                                        if (isNative) {
                                            ...
                                            if (path) {
                                                const blob = await readUriAsBlob(path);
                                                const file = new File([blob], 'gallery_invoice.jpg', { type: 'image/jpeg' });
                                                handleAIScan(file);
                                            }
                                        } else {
